# Changelog

## v0.2.0 — unreleased

- Added one client release manifest and a fail-closed distribution gate. A
  floating GitHub `latest` link, stale package version, incomplete skill set,
  or unapproved client send now fails verification.
- RC4 makes first-run readiness depend on runtime rule/skill discovery and no
  longer asks a beginner session to read or create diagnostic files.
- Added a screenshot-backed troubleshooting note for `5.6 Sol Light`; model
  selection stays under the owner's control.
- RC3 replaces the abbreviated ProblemOS Lite capsule with the complete
  Problem Solving Loop, makes numbered navigation mandatory after every
  non-trivial result, and adds an explicit no-bridge boundary.
- RC2 removes Python and developer-tool installation from the beginner path;
  Codex now checks a static manifest directly with project-file tools.
- Replaced the skill-count approach with one beginner-first `START` / `НАЧАТЬ` route.
- Organized skills into required core, on-demand work modules, and opt-in support.
- Added human-readable preflight and local-only support preview scripts.
- Added beginner instructions, local progress template, and explicit no-tracking rules.
- Added an evidence-backed acceptance matrix; platform-specific testing remains gated.

## v0.1.0 — 2026-08-06

- Bilingual English and Russian onboarding.
- Repository-local `AGENTS.md` with calm working and permission rules.
- Private `ABOUT_ME.md` flow backed by a public template and `.gitignore`.
- Three repository-local Codex skills: calm workflow, context interview, and
  privacy gate.
- Public-surface validator, privacy guide, contribution rules, and MIT license.
